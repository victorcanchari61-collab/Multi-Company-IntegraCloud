using Backend.Application.ERP.Commands.Categories;
using Backend.Application.ERP.Queries.Categories;
...
[Authorize]
public sealed class CategoriesController(IMediator mediator, TenantContext tenant) : ControllerBase
{
    [HttpGet]
    ...
    public async Task<IActionResult> SetStatus(Guid id, SetCategoryStatusCommand command, CancellationToken ct)
    {
        if (tenant.CompanyId is not { } companyId)
            return NoTenant();
        var result = await mediator.Send(command with { Id = id, CompanyId = companyId }, ct);
