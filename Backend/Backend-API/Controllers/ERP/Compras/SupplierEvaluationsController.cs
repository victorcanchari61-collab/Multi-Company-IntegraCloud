using Backend.Application.ERP.Commands.SupplierEvaluations;
using Backend.Application.ERP.Queries.SupplierEvaluations;
...
[ApiController]
[Route("api/erp/supplier-evaluations")]
[Authorize]
public sealed class SupplierEvaluationsController(IMediator mediator, TenantContext tenant) : ControllerBase
{
    [HttpGet]
    public async Task<IActionResult> GetAll([FromQuery] Guid? supplierId, CancellationToken ct)
    {
        ...
        var result = await mediator.Send(new GetSupplierEvaluationsQuery(companyId, supplierId), ct);
        return Ok(result.Value);
    }

    [HttpPost]
    public async Task<IActionResult> Create(CreateSupplierEvaluationCommand command, CancellationToken ct)
    {
        if (tenant.CompanyId is not { } companyId) return NoTenant();
    ...
    {
        if (tenant.CompanyId is not { } companyId) return NoTenant();
        var result = await mediator.Send(new GetSupplierEvaluationByIdQuery(id, companyId), ct);
        return result.IsSuccess ? Ok(result.Value) : ToError(result.Error!.Value);
    }

    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, UpdateSupplierEvaluationCommand command, CancellationToken ct)
    {
        if (tenant.CompanyId is not { } companyId) return NoTenant();
    ...
    {
        if (tenant.CompanyId is not { } companyId) return NoTenant();
        var result = await mediator.Send(new DeleteSupplierEvaluationCommand(id, companyId), ct);
        return result.IsSuccess ? NoContent() : ToError(result.Error!.Value);
    }
