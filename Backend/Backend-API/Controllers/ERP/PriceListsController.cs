using Backend.Application.ERP.Commands.PriceLists;
using Backend.Application.ERP.Queries.PriceLists;
...
[ApiController]
[Route("api/erp/price-lists")]
[Authorize]
public sealed class PriceListsController(IMediator mediator, TenantContext tenant) : ControllerBase
{
    [HttpGet]
    public async Task<IActionResult> GetAll(CancellationToken ct)
    {
        if (tenant.CompanyId is not { } companyId)
            return NoTenant();
        var result = await mediator.Send(new GetPriceListsQuery(companyId), ct);
        return Ok(result.Value);
    }

    [HttpPost]
    public async Task<IActionResult> Create(CreatePriceListCommand command, CancellationToken ct)
    {
        if (tenant.CompanyId is not { } companyId)
            return NoTenant();
    ...
    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, UpdatePriceListCommand command, CancellationToken ct)
    {
        if (tenant.CompanyId is not { } companyId)
            return NoTenant();
    ...
    public async Task<IActionResult> SetStatus(Guid id, SetPriceListStatusCommand command, CancellationToken ct)
    {
