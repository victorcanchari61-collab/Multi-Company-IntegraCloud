using Backend.Application.ERP.Commands.SalesCommissions;
using Backend.Application.ERP.Queries.SalesCommissions;
...
[ApiController]
[Route("api/erp/sales-commissions")]
[Authorize]
public sealed class SalesCommissionsController(IMediator mediator, TenantContext tenant) : ControllerBase
{
    [HttpGet]
    public async Task<IActionResult> GetAll(CancellationToken ct)
    {
        if (tenant.CompanyId is not { } companyId) return NoTenant();
        var result = await mediator.Send(new GetSalesCommissionsQuery(companyId), ct);
        return Ok(result.Value);
    }

    [HttpPost]
    public async Task<IActionResult> Create(CreateSalesCommissionCommand command, CancellationToken ct)
    {
        if (tenant.CompanyId is not { } companyId) return NoTenant();
    ...
    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, UpdateSalesCommissionCommand command, CancellationToken ct)
    {
        if (tenant.CompanyId is not { } companyId) return NoTenant();
    ...
    public async Task<IActionResult> SetStatus(Guid id, SetSalesCommissionStatusCommand command, CancellationToken ct)
    {
        if (tenant.CompanyId is not { } companyId) return NoTenant();
        var result = await mediator.Send(command with { Id = id, CompanyId = companyId }, ct);
        return result.IsSuccess ? NoContent() : ToError(result.Error!.Value);
