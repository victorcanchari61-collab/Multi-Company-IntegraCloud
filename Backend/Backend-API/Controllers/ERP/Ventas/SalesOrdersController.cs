using Backend.Application.ERP.Commands.SalesOrders;
using Backend.Application.ERP.Queries.SalesOrders;
...
[ApiController]
[Route("api/erp/sales-orders")]
[Authorize]
public sealed class SalesOrdersController(IMediator mediator, TenantContext tenant) : ControllerBase
{
    [HttpGet]
    public async Task<IActionResult> GetAll(CancellationToken ct)
    {
        if (tenant.CompanyId is not { } companyId) return NoTenant();
        var result = await mediator.Send(new GetSalesOrdersQuery(companyId), ct);
    ...
    {
        if (tenant.CompanyId is not { } companyId) return NoTenant();
        var result = await mediator.Send(new GetSalesOrderByIdQuery(id, companyId), ct);
    ...
    public async Task<IActionResult> UpdateStatus(Guid id, UpdateSalesOrderStatusCommand command, CancellationToken ct)
    {
        if (tenant.CompanyId is not { } companyId) return NoTenant();
