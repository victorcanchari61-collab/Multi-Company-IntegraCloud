...
public sealed class RolesController(IMediator mediator) : ControllerBase
{
    ...
    [HttpGet("tree")]
    public async Task<IActionResult> GetTree(Guid companyId, CancellationToken ct)
    {
        var result = await mediator.Send(new GetRoleTreeByCompanyQuery(companyId), ct);
        return Ok(result);
    }
    ...
    [HttpPost("{roleId:guid}/restrictions")]
    public async Task<IActionResult> AddRestriction(Guid companyId, Guid roleId,
        AddRoleRestrictionCommand command, CancellationToken ct)
    {
        var result = await mediator.Send(command with { RoleId = roleId }, ct);
        return result.IsSuccess ? NoContent() : ToError(result.Error!.Value);
    }

    [HttpDelete("{roleId:guid}/restrictions/{restrictedKey}")]
    public async Task<IActionResult> RemoveRestriction(Guid companyId, Guid roleId,
        string restrictedKey, CancellationToken ct)
    {
        var result = await mediator.Send(new RemoveRoleRestrictionCommand(roleId, restrictedKey), ct);
        return result.IsSuccess ? NoContent() : ToError(result.Error!.Value);
    }
