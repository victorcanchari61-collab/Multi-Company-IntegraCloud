...
public sealed class UsersController(IMediator mediator) : ControllerBase
{
    ...
    [HttpPost("{userId:guid}/restrictions")]
    public async Task<IActionResult> AddRestriction(Guid companyId, Guid userId,
        AddUserRestrictionCommand command, CancellationToken ct)
    {
        var result = await mediator.Send(command with { UserId = userId }, ct);
        return result.IsSuccess ? NoContent() : ToError(result.Error!.Value);
    }

    [HttpGet("{userId:guid}/restrictions")]
    public async Task<IActionResult> GetRestrictions(Guid companyId, Guid userId, CancellationToken ct)
    {
        var result = await mediator.Send(new GetUserRestrictionsQuery(userId), ct);
        return result.IsSuccess ? Ok(result.Value) : ToError(result.Error!.Value);
    }

    [HttpDelete("{userId:guid}/restrictions/{restrictedKey}")]
    public async Task<IActionResult> RemoveRestriction(Guid companyId, Guid userId,
        string restrictedKey, CancellationToken ct)
    {
        var result = await mediator.Send(new RemoveUserRestrictionCommand(userId, restrictedKey), ct);
        return result.IsSuccess ? NoContent() : ToError(result.Error!.Value);
    }
