...
interface DataTableProps<TData> {
  // TValue = any: acepta tanto ColumnDef<T, unknown> (catálogo) como la salida de
  // createColumnHelper (ColumnDef<T, string|number>) usada en inventario. Idiomático en TanStack.
...
export function DataTable<TData>({
  ...
  const [columnOrder, setColumnOrder] = useState<ColumnOrderState>(() =>
    columns.map((c) => {
      const def = c as { id?: string; accessorKey?: string }
      return (def.id ?? def.accessorKey ?? '') as string
    }),
  )
          ...
          {loading ? (
            ...
            rows.map((row, i) => (
              <div
                ...
                className={cn(
                  'animate-dt-fade-in rounded-lg bg-card p-3 shadow-sm ring-1 ring-black/5',
                  onRowClick && 'cursor-pointer',
                )}
