...
function getNameColumn<T extends NamedEntity>(): ColumnDef<T, unknown> {
  return {
    ...
    accessorKey: 'name',
    header: 'Nombre',
    // En móvil el nombre ya se muestra como título de la card (mobileTitle).
    meta: { label: 'Nombre', hideOnMobile: true },
...
function getStatusColumn<T extends NamedEntity>(): ColumnDef<T, unknown> {
  return {
    ...
    header: 'Estado',
    meta: { label: 'Estado' },
    cell: ({ row }) => <StatusBadge isActive={row.original.isActive} mutedInactive />,
  }
}
