...
const schema = z.object({
  ...
  unitCost: z.string().optional().refine((v) => !v || Number(v) >= 0, 'Debe ser ≥ 0'),
...
export function StockMovementFormDialog({ open, onOpenChange, warehouses, onSave, saving }: Props) {
  const form = useForm<FormValues>({
    ...
    defaultValues: { warehouseId: '', productId: '', movementType: '', quantity: '', unitCost: '', notes: '' },
  })
  ...
  const onSubmit = (values: FormValues) => {
    onSave({
      ...
      unitCost: values.unitCost && values.unitCost.trim() !== '' ? Number(values.unitCost) : null,
