...
export function StockMovementsSection({ filterMovementTypes }: { filterMovementTypes?: string[] }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <DataTable
        columns={columns}
        data={filterMovementTypes ? (movements ?? []).filter((m) => filterMovementTypes.includes(m.movementType)) : (movements ?? [])}
        loading={isLoading}
        getRowId={(r) => r.id}
