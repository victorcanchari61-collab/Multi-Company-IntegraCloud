...
/** Descarga un archivo (PDF, Excel, etc.) desde la API y abre en nueva pestaña. */
export async function downloadBlob(endpoint: string) {
  const { accessToken } = useAuthStore.getState()
  const response = await fetch(`${env.API_URL}${endpoint}`, {
    headers: accessToken ? { Authorization: `Bearer ${accessToken}` } : {},
  })
  if (!response.ok) throw new ApiError(response.status, 'download_error', 'Error al descargar el archivo.')
