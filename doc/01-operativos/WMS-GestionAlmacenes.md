...
### 3. Inventario físico
> Puede apoyarse en el módulo de Inventario del ERP para los movimientos y ajustes. Estado de implementación cruzada.

| Submódulo | Estado | ¿Dónde? | Descripción |
|-----------|--------|---------|-------------|
| Stock por ubicación | ❌ | WMS | Cantidades exactas por ubicación física (requiere ubicaciones primero) |
| Conteos cíclicos | ❌ | WMS | Inventarios periódicos por rotación ABC |
| Inventario general | ❌ | WMS | Conteo total programado del almacén |
| Diferencias y ajustes físicos | ⚠️ Parcial | ERP | Los ajustes existen en ERP (ADJUSTMENT_POSITIVE/NEGATIVE), falta el flujo de conteo → diferencia → aprobación → ajuste |
| Trazabilidad de movimientos | ✅ | ERP | Kárdex contable registra todas las entradas, salidas y saldos valorizados |
